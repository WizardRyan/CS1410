...
{  
   ...
   {  
	  ...
	  LPCTSTR pszClassName = LPTSTR((LPCTSTR)"CCC_WIN");
   ...
   char title[80] = {};
   GetModuleFileNameA(hInstance, title, sizeof(title));

   HWND hwnd = CreateWindowA("CCC_WIN",
      title,
...
{  
   ...
   child_hwnd = CreateWindowA("static",
      NULL,
   ...
   if (!_hbm)
   {  
      MessageBoxA(hwnd, (LPSTR)"Memory problems in Bitmap Creation",
...
{  
   ...
   TextOutA(_hdc, user_to_disp_x(x), user_to_disp_y(y), t, lstrlenA(t));
   SetBkMode(_bitmap_hdc, TRANSPARENT);
   TextOutA(_bitmap_hdc, user_to_disp_x(x), user_to_disp_y(y), t, lstrlenA(t));
}
...
{  
   ...
   SIZE sz;
   GetTextExtentPoint32A(child_hdc, t, strlen(t), &sz);
   _mlength = sz.cx;
   GetTextExtentPoint32A(child_hdc, " ", 1, &sz);
   _mlength += sz.cx;
